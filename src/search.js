import { getEnv, getEnabledEngines } from "./env.js";
...
async function searchSingle(engineName, query, timeout) {
  ...
  try {
    ...
    if (e.name === "AbortError") {
      ...
      console.error(`[${engineName}] ${e.message}`);
    }
    return [];
  }
}
...
export async function searchWithFallback(query, enginesOverride) {
  const env = getEnv();
  ...
  const engines =
    enginesOverride && enginesOverride.length
      ? enginesOverride.map((n) => n.toLowerCase()).filter((n) => enabled.includes(n))
      : enabled;
